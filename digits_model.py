...
class DigitDataset(data.Dataset):
    def __init__(self, path, train=True, target=7, transform=None):
        _dataset = torchvision.datasets.MNIST(root=path, train=train, download=True)
...
d_train = DigitDataset('mnist', train=True, transform=transforms)
...
for _e in range(epochs):
    ...
    for x_train, y_train in train_tqdm:
        ...
        train_tqdm.set_description(
            f"Epoch [{_e + 1}/{epochs}], loss_mean_gen={loss_mean_gen:.3f}, loss_mean_dis={loss_mean_dis:.3f}")
